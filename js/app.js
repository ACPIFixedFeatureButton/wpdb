                ...
                <ul class="space-y-1">
                    ${brand.devices.map(d => `
                        <li 
                            ...
                            id="nav-${d.codename}"
                            class="sidebar-item px-3 py-1.5 cursor-pointer text-sm font-medium text-gray-600 border-l-4 border-transparent transition-all duration-300 ease-metro hover:text-black hover:bg-white hover:border-gray-200 truncate"
        ...
        brand.devices.forEach((d, index) => {
            ...
            html += `
                <div 
                    ...
                    class="group relative aspect-[16/9] bg-white dark:bg-[#2a2a2a] cursor-pointer shadow-sm animate-tile-enter overflow-hidden flex flex-col"
...
function goToHome(updateHistory = true) {
    ...
    document.querySelectorAll('.sidebar-item').forEach(el => {
        el.classList.remove('border-wp-blue', 'bg-white', 'text-black', 'shadow-sm');
...
function openDevice(codename, updateHistory = true) {
    ...
    if(activeItem) {
        ...
        activeItem.classList.add('border-wp-blue', 'bg-white', 'text-black', 'shadow-sm');
    }
    ...
    const guide = device.guide || [
        { title: "Generic Guide", text: "Please use Windows Device Recovery Tool or Thor2 to flash this .ffu file." },
        { title: "Warning", text: "Always backup your data before flashing. Battery should be at least 50%." }
    ];
                            ...
                            <div class="bg-gray-50 p-4 border border-gray-200 rounded-sm">
                                ...
                                ${fw.files ? 
                                    `<div class="space-y-2">
                                        ${fw.files.map(f => `
                                            <div class="flex items-center gap-2">
                                                <button onclick="copyLink('${f.url}')" class="p-3 bg-gray-200 text-gray-600"><i class="fa-regular fa-copy"></i></button>
                                                <a href="${f.url}" class="block w-full text-center bg-white border border-gray-300 py-3 text-sm font-bold text-gray-700 hover:border-wp-blue hover:text-wp-blue transition-colors">
                                                    ${f.type} (${f.size})
                                                </a>
                                            </div>
                                        `).join('')}
                                    ...
                                    : 
                                    `<div class="flex items-center gap-2">
                                        <button onclick="copyLink('${fw.url}')" class="p-3 bg-gray-200 text-gray-600"><i class="fa-regular fa-copy"></i></button>
                                        <a href="${fw.url}" class="block w-full bg-black text-white text-center py-3 font-bold uppercase tracking-wide shadow-md active:scale-95 transition-transform">
                                            Download (${fw.size})
                                        </a>
                                    </div>`
                                }
                                        ...
                                        <td class="py-5 px-6 align-top text-right pt-6">
                                            ${fw.files ? 
                                                `<div class="flex flex-col items-end gap-2">
                                                    ${fw.files.map(f => `
                                                        <div class="flex items-center justify-end gap-2 w-full">
                                                            ...
                                                            <a href="${f.url}" class="group/btn flex items-center gap-3 pl-4 pr-3 py-1.5 border border-gray-200 hover:border-wp-blue bg-white hover:bg-wp-blue transition-all duration-200 rounded-sm">
                                                ...
                                                `<div class="inline-flex items-center gap-2 justify-end">
                                                    ...
                                                    <a href="${fw.url}" class="inline-flex items-center gap-3 bg-black hover:bg-wp-blue text-white px-6 py-3 transition-colors shadow-lg hover:shadow-xl active:scale-95 duration-200">
    ...
    if (!hasResults) {
        if(!document.getElementById('empty-msg')) {
            ...
            msg.innerHTML = `
                ...
                <div class="text-sm font-light">No results found.</div>
            `;
